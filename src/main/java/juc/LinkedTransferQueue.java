...
public class LinkedTransferQueue<E> extends AbstractQueue<E>
            ...
            for (Node h = head, p = h; p != null; ) { // find & match first node
                ...
                if (item != p && (item != null) == isData) { // unmatched
                    ...
                    if (p.casItem(item, e)) { // match
                        ...
                        return juc.LinkedTransferQueue.cast(item);
                    }
                }
                Node n = p.next;
    ...
    final class Itr implements Iterator<E> {
        ...
        private void advance(Node prev) {
            ...
            for (Node p = prev, s, n; ; ) {
                ...
                if (s.isData) {
                    if (item != null && item != s) {
                        nextItem = juc.LinkedTransferQueue.cast(item);
                        nextNode = s;
                        return;
                    }
                } else if (item == null)
    ...
    static final class LTQSpliterator<E> implements Spliterator<E> {
        ...
        final juc.LinkedTransferQueue<E> queue;
        Node current;    // current node; null until initialized
        int batch;          // batch size for splits
        boolean exhausted;  // true when no more nodes

        LTQSpliterator(juc.LinkedTransferQueue<E> queue) {
            this.queue = queue;
        }

        public Spliterator<E> trySplit() {
            Node p;
            final juc.LinkedTransferQueue<E> q = this.queue;
            int b = batch;
     ...
     * {@link juc.BlockingQueue#offer(Object, long, TimeUnit)
     * BlockingQueue.offer})
     * @throws NullPointerException if the specified element is null
     */
     ...
     * {@link juc.BlockingQueue#remainingCapacity()
